...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS queue (username TEXT, timestamp BIGINT, estimatedWait TEXT)")
  }
  ...
  override def addStudentToQueue(student: StudentInQueue): Unit = {
    ...
    statement.setString(3, student.estimatedWait)
  ...
  override def getQueue: List[StudentInQueue] = {
    ...
    while (result.next()) {
      ...
      val estimatedWait = result.getString("estimatedWait")
      queue = new StudentInQueue(username, timestamp, estimatedWait) :: queue
    }
