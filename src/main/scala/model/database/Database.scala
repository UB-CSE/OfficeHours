...
class Database extends DatabaseAPI{
  ...
  def setupTable(): Unit = {
    ...
    statement.execute("CREATE TABLE IF NOT EXISTS queue (username TEXT, timestamp BIGINT, purpose TEXT)")
  }
  ...
  override def addStudentToQueue(student: StudentInQueue): Unit = {
    ...
    statement.setString(3, student.purpose)
  ...
  override def getQueue: List[StudentInQueue] = {
    ...
    while (result.next()) {
      ...
      val purpose = result.getString("purpose")
      queue = new StudentInQueue(username, timestamp, purpose) :: queue
    }
