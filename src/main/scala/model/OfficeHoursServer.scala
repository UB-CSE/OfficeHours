...
class EnterQueueListener(server: OfficeHoursServer) extends DataListener[String] {
  override def onData(socket: SocketIOClient, data: String, ackRequest: AckRequest): Unit = {
    println(" am server enter queue")
    val username = data.split(",")(0)
    val issue = data.split(",")(1)
    server.database.addStudentToQueue(StudentInQueue(username, System.nanoTime(),issue))
